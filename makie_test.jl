...
# ╔═╡ 2bc04a22-42ec-11ec-2971-9b21113d7aee
begin
using LinearAlgebra
	using GLMakie
	GLMakie.activate!()
...
# ╔═╡ c754921b-5634-445e-9b36-fe320418cf2f
upper(x,y) = 0.5 > x.^2 + y.^2 ? sqrt(0.5 - x.^2 - y.^2) : NaN

# ╔═╡ 9c6be71b-ca35-4cfe-a205-c5d8c2f17009
#surface(-0.5:0.1:0.5, -0.5:0.1:0.5,  (x,y) -> -1 * upper(x,y) )

# ╔═╡ 864847d4-0547-44ac-b87c-06599719a902
begin
...
volume(cube_with_holes, algorithm = :iso, isorange = 0.02, isovalue = 2, absorption = true)

end

# ╔═╡ 36b575aa-d848-4be4-abc7-9cdeb246c00e
cube_with_holes

# ╔═╡ 5c53cb17-c7a9-4248-b5df-c2b498d59d65
Point3f0[
    (1.,0.,0.), 1/sqrt(2) .* (1.,1.,0), 1/sqrt(3) .* (1.,1.,1.), (-1.,0.,0.), (0.,0.,1.)
  ]

# ╔═╡ 0a6c9399-1dae-45f9-b168-fc36799a8ed1
begin 
	n = 50
...
# ╔═╡ e34db531-af42-4c7e-963a-43526a85e032
begin 

surface(
  x,y,z,
  show_axis = false,
  color = fill(RGBA(1.,1.,0.4,1.0), n, n),
 # limits = HyperRectangle(Vec3f0(-1.01), Vec3f0(1.01))
)
scatter(
  [1,  1/sqrt(2), 1/sqrt(3)], [0, 1/sqrt(2), 1/sqrt(3)], [0, 0, 1/sqrt(3)],
  color = :blue, markersize = 0.05, show_axis = false
)
end

# ╔═╡ 7347a596-6c08-4682-8564-a318cb9c612c
RGBA(z)

# ╔═╡ ac716620-964a-4e8b-9baa-a87299bb2896
A = Colors.colormap("RdBu")

# ╔═╡ 0bb41a4b-8413-4837-8753-84ab29e3925f
X = RGBA.(A)

# ╔═╡ 694cfc9c-3ad9-4deb-9a42-c56adc687f65
[X[i].alpha = 0.3 for i = 1:length(X)]

# ╔═╡ d3321153-10a8-4d8b-8068-da96496db3d3
Y = X[3]

# ╔═╡ 8e37e892-8bff-4fe1-a1b2-35401ec9a673
setfield!(Y, :alpha, 0.3)

# ╔═╡ fcbb5595-dd12-40a3-81f4-8d17f104517d
typeof(Gray)

# ╔═╡ a1bf26f4-8494-4971-9b7e-f495c465b73b
typeof(Gray.(X))

# ╔═╡ 7a3ba07e-ecd6-4190-a685-c778de328d32
gray.(Gray.(X))
